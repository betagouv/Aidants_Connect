...
class BaseAidantRequestFormSet(BaseHabilitationRequestFormSet):
    ...
    def get_objects(self):
        for it in self.get_queryset().all():
            yield ProfileCardAidantRequestPresenter(self.organisation, it)
    ...
    def is_empty(self):
        for form in self.forms:
            ...
            if not form.is_valid() or form.is_valid() and form.cleaned_data:
                return False
    ...
    def validate_unique(self):
        emails = {
            existing_email: []
            for existing_email in self.organisation.aidant_requests.values_list(
                "email", flat=True
            )
        }
        manager_email = (
