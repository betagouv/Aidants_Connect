...
class ManagerForm extends BaseController {
  ...
  onManagerItsMeBtnClicked() {
    Object.keys(this.issuerDataValue).forEach((key) => {
      this.element.querySelector(`[name$='${key}']`).value =
        this.issuerDataValue[key];
    });
  }
  ...
  onAddressSameAsOrgChanged({ target: { value } }) {
    ...
    this.mutateRequirement(
      !JSON.parse(value.toLowerCase()),
      this.autcompleteInputTarget
    );
    this.mutateRequirement(
      !JSON.parse(value.toLowerCase()),
      this.zipcodeInputTarget
    );
    this.mutateRequirement(
      !JSON.parse(value.toLowerCase()),
...
aidantsConnectApplicationReady.then((application) =>
  application.register("manager-form", ManagerForm)
);
