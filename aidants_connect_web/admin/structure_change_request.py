...
class StructureChangeRequestAdmin(VisibleToAdminMetier, ModelAdmin):
    ...
    search_fields = ("email", "new_email", "organisation__name")
    ...
    fieldsets = (
        (
            "Demande",
            {
                "fields": ("email", "organisation", "new_email", "status"),
            },
        ),
        (
            "Dates",
    ...
    def mark_validated(self, request, queryset: QuerySet):
        count = sum(1 for req in queryset if req.validate_and_create_aidant())
        self.message_user(
            request,
