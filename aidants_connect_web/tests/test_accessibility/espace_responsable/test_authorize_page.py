...
class AuthorizePageAccessibilityTests(AccessibilityTestCase):
    def setUp(self):
        super().setUp()
    ...
    async def navigate_to_authorize_page(self):
    ...
    async def test_accessibility(self):
        await self.navigate_to_authorize_page()
        await self.check_accessibility(page_name="authorize", strict=True)

    @async_test
    async def test_title_is_correct(self):
        await self.navigate_to_authorize_page()
        await expect(self.page).to_have_title("Sélection de l'usager - Aidants Connect")
