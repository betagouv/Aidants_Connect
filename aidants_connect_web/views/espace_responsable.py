...
# Session flag used to remember that the referent has clicked
# "Faire plus tard" and should not be auto-redirected to the OTP tunnel
# for the rest of this session.
OTP_TUNNEL_DISMISSED_SESSION_KEY = "otp_tunnel_dismissed"
...
def referent_should_enter_otp_tunnel(request) -> bool:
    ...
    OTP onboarding tunnel (no TOTP device configured yet and tunnel not
    ...
    if not user.is_responsable_structure:
        return False
    if user.has_a_totp_device_confirmed_or_not:
        return False
    return not request.session.get(OTP_TUNNEL_DISMISSED_SESSION_KEY, False)
...
class HomeView(DetailView, FormView):
    ...
    def dispatch(self, request, *args, **kwargs):
        self.referent: Aidant = request.user
        # Redirect a referent without any TOTP device to the OTP onboarding
        # tunnel, unless they have explicitly dismissed it during this session.
        if request.method == "GET" and referent_should_enter_otp_tunnel(request):
            return redirect("espace_referent:otp_tunnel_welcome")
        return super().dispatch(request, *args, **kwargs)
...
@responsable_logged_required
class OtpTunnelWelcomeView(TemplateView):
    """First page of the OTP onboarding tunnel shown to a referent who has
    not yet configured any TOTP device."""

    template_name = "aidants_connect_web/espace_responsable/otp_tunnel/welcome.html"

    def dispatch(self, request, *args, **kwargs):
        self.referent: Aidant = request.user
        # If the referent already has an OTP device, no need to onboard them.
        if self.referent.has_a_totp_device_confirmed_or_not:
...
class OtpTunnelStepViewMixin:
    """Shared dispatch logic for OTP tunnel steps: only referents without a
    TOTP device may reach a step. Subclasses must set ``step_number``."""

    step_number: int = 1

    def dispatch(self, request, *args, **kwargs):
        self.referent: Aidant = request.user
        # Referents who already have a TOTP device should not see the tunnel.
        if self.referent.has_a_totp_device_confirmed_or_not:
            return redirect("espace_referent:home")
        return super().dispatch(request, *args, **kwargs)
    ...
    def get_context_data(self, **kwargs):
        return {
            **super().get_context_data(**kwargs),
            "referent": self.referent,
            "wizard_step": self.step_number,
            "wizard_total_steps": OTP_TUNNEL_TOTAL_STEPS,
        }


@responsable_logged_required
class OtpTunnelDownloadAppView(OtpTunnelStepViewMixin, TemplateView):
    """Step 1/3 of the OTP onboarding tunnel: explain how to download an
    authentication application on the referent's mobile phone."""
...
@responsable_logged_required
class OtpTunnelScanQrCodeView(OtpTunnelStepViewMixin, FormView):
    """Step 2/3 of the OTP onboarding tunnel: display a QR code and ask the
    referent to enter the 6-digit code generated by their authenticator app
    to confirm the pairing.

    Mirrors ``AddAppOTPToAidant`` but targets the current referent: the TOTP
    device is built in memory, persisted in session between GET and POST,
    then saved (confirmed) when the OTP code is valid. The referent is then
    redirected to the congratulations page.
    """
    ...
    form_class = AddAppOTPToAidantForm
    step_number = 2
    success_url = reverse_lazy("espace_referent:otp_tunnel_congratulations")
    ...
    def get_context_data(self, **kwargs):
        return {
            **super().get_context_data(**kwargs),
            "otp_device_qr_code_href": (
                f"data:image/png;base64,{self._get_qr_code_base64()}"
            ),
            "otp_device_secret_key": base64.b32encode(self.otp_device.bin_key).decode(
                "ascii"
            ),
        }

    def _get_qr_code_base64(self) -> str:
        stream = BytesIO()
        img = qrcode.make(self.otp_device.config_url, box_size=7, border=4)
        img.save(stream, "PNG")
        return base64.b64encode(stream.getvalue()).decode("utf-8")
...
@responsable_logged_required
class OtpTunnelCongratulationsView(TemplateView):
    """Step 3/3 of the OTP onboarding tunnel: congratulate the referent on
    pairing their authenticator app and invite them to enter the referent
    space.

    Unlike the other steps, this page is only meaningful for a referent who
    has already configured a TOTP device. Referents without a device are
    sent back to the beginning of the tunnel.
    """
    ...
    def dispatch(self, request, *args, **kwargs):
        self.referent: Aidant = request.user
        if not self.referent.has_a_totp_device_confirmed_or_not:
            return redirect("espace_referent:otp_tunnel_download_app")
        return super().dispatch(request, *args, **kwargs)
