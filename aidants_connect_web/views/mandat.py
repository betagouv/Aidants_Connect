...
class NewMandatRecap(RequireConnectionObjectMixin, FormView):
    ...
    def form_valid(self, form):
        ...
        try:
            ...
            # Create a mandat
            mandat = Mandat.objects.create(**self._get_mandat_create_kwargs(fixed_date))

            # Add a Journal 'create_attestation' action
            Journal.log_attestation_creation(
                **self._get_connection_create_kwargs(fixed_date, mandat)
            )
    ...
    def _get_mandat_create_kwargs(self, fixed_date: datetime):
        result = {
            "organisation": self.aidant.organisation,
            "usager": self.connection.usager,
        ...
        if self.connection.mandat_is_remote:
            result["consent_request_id"] = self.connection.consent_request_id
            result["remote_constent_method"] = RemoteConsentMethodChoices[
                self.connection.remote_constent_method
            ]

        return result

    def _get_connection_create_kwargs(self, fixed_date: datetime, mandat: Mandat):
        result = {
            "aidant": self.aidant,
            "usager": self.connection.usager,
            ...
            "attestation_hash": generate_attestation_hash(
                ...
                AuthorizationDurations.expiration(
                    self.connection.duree_keyword, fixed_date
                ),
            ),
            "mandat": mandat,
        ...
        if self.connection.mandat_is_remote:
            result["user_phone"] = self.connection.user_phone
            result["consent_request_id"] = self.connection.consent_request_id
            result["remote_constent_method"] = RemoteConsentMethodChoices[
                self.connection.remote_constent_method
            ]
