...
class NewHabilitationRequest extends BaseController {
  initialize() {
    ...
    if (
      this.formTarget.querySelectorAll('input[name*="__prefix__"]').length > 0
    ) {
  ...
  nbFormCountValueChanged(value) {
    ...
    value > 1
      ? this.showElement(this.rightFormsSectionTarget)
      : this.hideElement(this.rightFormsSectionTarget);
  }
  ...
  onPartialSubmit() {
    ...
    const url = this.editingValue
      ? Urls.espaceResponsableAidantNewJsEdit({
          form_idx: this.leftFormIdxValue,
        })
      : Urls.espaceResponsableAidantNewJs();
    fetch(url, { method: "POST", body: new FormData(this.formTarget) })
      .then(async (response) => {
        if (response.status === 200) {
          ...
          // Form is ok
          this.rightFormsInsertTarget.insertAdjacentHTML(
            "beforeend",
            await response.text(),
          );
          this.validateAndResetLeftForm();
  ...
  editForm(formIdx) {
    ...
    fetch(Urls.espaceResponsableAidantNewJsEdit({ form_idx: formIdx }), {
      method: "POST",
      body: new FormData(this.formTarget),
    })
      .then(async (response) => {
        this.rightFormsInsertTarget
          .querySelectorAll(
