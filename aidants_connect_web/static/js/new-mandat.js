...
(() => {
  ...
  class MandateFormController extends BaseController {
    ...
    scopesValueChanged(val) {
      this.mutateVisibility(
        this.scopesValue[this.bdfWarningValue],
        this.bdfWarningTarget,
      );
    }
    ...
    consentMethodValueChanged(value) {
      this.dispatch(REMOTE_METHOD_EVENT_NAME, {
        detail: { method: value.trim() },
        prefix: REMOTE_METHOD_EVENT_NAME_PREFIX,
      });
    }
    ...
    isRemoteValueChanged(value) {
      ...
      this.remoteLabelTextTargets.forEach((elt) =>
        this.mutateVisibility(value, elt),
      );
      this.requiredInputTargets.forEach((elt) =>
        this.mutateRequirement(value, elt),
      );

      this.dispatch(IS_REMOTE_METHOD_EVENT_NAME, {
        detail: { isRemote: value },
        prefix: IS_REMOTE_METHOD_EVENT_NAME_PREFIX,
      });
    }
  ...
  class RemoteMethodController extends BaseController {
    connect() {
      if (this.hasRequiredInputsTarget) {
        ...
        this.mandateFormControllerElt.addEventListener(
          `${REMOTE_METHOD_EVENT_NAME_PREFIX}:${REMOTE_METHOD_EVENT_NAME}`,
          this.boundRemoteMethodTriggered,
        );

        this.boundIsRemoteTriggered = this.isRemoteTriggered.bind(this);
        this.mandateFormControllerElt.addEventListener(
          `${IS_REMOTE_METHOD_EVENT_NAME_PREFIX}:${IS_REMOTE_METHOD_EVENT_NAME}`,
          this.boundIsRemoteTriggered,
        );
      }
    }
    ...
    remoteMethodTriggered(event) {
      ...
      this.requiredInputsTarget
        .querySelectorAll("input")
        .forEach((elt) => this.mutateRequirement(state, elt));
    }
    ...
    isRemoteTriggered(event) {
      ...
      this.requiredInputsTarget
        .querySelectorAll("input")
        .forEach((elt) => this.mutateRequirement(event.detail.isRemote, elt));
    }
