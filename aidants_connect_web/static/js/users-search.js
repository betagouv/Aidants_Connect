...
(() => {
  ...
  class SearchController extends BaseController {
    ...
    searchTokenValueChanged(value) {
      const event = new CustomEvent(SEARCH_EVENT_NAME, {
        detail: { term: value.trim() },
      });
      this.element.dispatchEvent(event);
    }

    static targets = ["searchBar", "item", "searchInput"];
  ...
  class SearchItemController extends BaseController {
    ...
    connect() {
      ...
      document
        .querySelector("[data-controller='search']")
        .addEventListener(SEARCH_EVENT_NAME, this.boundFilter);
    }
    ...
    filter(event) {
      ...
      const hasMatchingTerms = this.searchTerms.some((term) =>
        localeIncludes(term, searchTerm),
      );
    ...
    disconnect() {
      document
        .querySelector("[data-controller='search']")
        .removeEventListener(SEARCH_EVENT_NAME, this.boundFilter);
    }
