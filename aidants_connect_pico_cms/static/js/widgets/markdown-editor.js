export class MarkdownEditor extends Stimulus.Controller {
  initialize() {
    ...
    for (const key in this.textareaContainerTarget.dataset) {
  ...
  previewRender(plainText, preview) {
    ...
    Object.entries(this.formDataEntries(plainText)).forEach(([k, v]) =>
      body.append(k, v.toString()),
    );
    fetch(this.renderEndpointValue, { method: "POST", body: body }).then(
      (response) => {
        if (response.ok) {
